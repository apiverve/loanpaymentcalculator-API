...
class loanpaymentcalculatorWrapper {
    ...
    async execute(query, callback) {
        ...
        var requiredParams = ["loanAmount","interestRate","loanTerm"];
        if (requiredParams.length > 0) {
            for (var i = 0; i < requiredParams.length; i++) {
                if (!query[requiredParams[i]]) {
                    throw new Error(`Required parameter [${requiredParams[i]}] is missing. See documentation: https://docs.apiverve.com/ref/loanpaymentcalculator`);
                }
            }
        }

...
class LoanpaymentcalculatorAPIClientError(Exception):
...
class ValidationError(LoanpaymentcalculatorAPIClientError):
...
class LoanpaymentcalculatorAPIClient:
    ...
    VALIDATION_RULES = {"loanAmount": {"type": "number", "required": True, "min": 0}, "interestRate": {"type": "number", "required": True, "min": 0, "max": 100}, "loanTerm": {"type": "integer", "required": True, "min": 1, "max": 50}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the LoanpaymentcalculatorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises LoanpaymentcalculatorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise LoanpaymentcalculatorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[LoanpaymentcalculatorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns LoanCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises LoanpaymentcalculatorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise LoanpaymentcalculatorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise LoanpaymentcalculatorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise LoanpaymentcalculatorAPIClientError(f"Request failed: {str(e)}")

// Package loanpaymentcalculator provides a Go client for the Loan Calculator API.
//
// For more information, visit: https://apiverve.com/marketplace/loanpaymentcalculator?utm_source=go&utm_medium=readme
...
// Request contains the parameters for the Loan Calculator API.
//
// Parameters:
//   - loanAmount (required): number - The loan amount [min: 0]
//   - interestRate (required): number - The interest rate (percentage) [min: 0, max: 100]
//   - loanTerm (required): integer - The loan term in years [min: 1, max: 50]
type Request struct {
	LoanAmount int `json:"loanAmount"` // Required
	InterestRate float64 `json:"interestRate"` // Required
	LoanTerm int `json:"loanTerm"` // Required
}
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"loanAmount": {Type: "number", Required: true, Min: float64Ptr(0)},
		"interestRate": {Type: "number", Required: true, Min: float64Ptr(0), Max: float64Ptr(100)},
		"loanTerm": {Type: "integer", Required: true, Min: float64Ptr(1), Max: float64Ptr(50)},
	}
...
// ResponseData contains the data returned by the Loan Calculator API.
